...
public class GameLogic : GameController
{
	...
	{ 
		...
		{
			puzzleSpheres [playerSelectionIndex].GetComponent<GvrAudioSource> ().Stop ();
			PuzzleFailure();
		}
	}
	...
	public void PuzzleFailure()
	{
		...
		for (int i = 0; i < puzzleSpheres.Length; i++) { 
			puzzleSpheres [i].GetComponent<LightUp> ().AestheticError (puzzleSpeed);
		}
